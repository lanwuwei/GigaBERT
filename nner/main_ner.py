...
if __name__ == '__main__':
    ...
    if args.bert_model == 'bert-base-multilingual-cased':
        ...
        model = XLMRobertaForNER.from_pretrained(args.bert_model,
                                           cache_dir=args.output_dir,
                                           num_labels=num_labels,
                                           output_hidden_states=True)  # if you want to get all layer hidden states
        ...
        model.load_state_dict(torch.load('/home/wuwei-lan/Documents/huggingface/saved_model/'+args.bert_model+'/pytorch_model.bin', map_location=device), strict=False) # pytorch ckpt file
    ...
    if args.bert_model == 'bert-base-multilingual-cased':
        ...
        if args.bert_model=='bibert-124k':
            lower_case_flag=False
        else:
            lower_case_flag=True
        print('lower_case_flag: ', lower_case_flag)
        tokenizer = BertTokenizer.from_pretrained('/home/wuwei-lan/Documents/huggingface/saved_model/'+args.bert_model+'/vocab.txt', do_lower_case=lower_case_flag) # bert vocab file
        tokenizer.bos_token = '[CLS]'
        tokenizer.eos_token = '[SEP]'
        tokenizer.unk_token = '[UNK]'
        tokenizer.sep_token = '[SEP]'
